<script lang="ts" setup>
  ...
  const resetSignaturePad = () => {
    if (canvas2D.value && signaturePad.value) {
      canvas2D.value.clearRect(0, 0, signaturePad.value.width, signaturePad.value.height);
      drawSignaturePad();
    }
  };
  ...
  useEventListener(signaturePad, ['mousedown', 'mousemove'], [[handleStart], [handleMove]]);
  useEventListener(signaturePad, ['touchstart', 'touchmove'], [[handleStart], [handleMove]], {
    passive: true,
  });
  useEventListener(signaturePad, ['mouseup', 'mouseleave', 'touchend'], handleEnd);
